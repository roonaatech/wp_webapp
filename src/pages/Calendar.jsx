...
const Calendar = () => {
    ...
    useEffect(() => {
        ...
        if (today.getFullYear() === currentDate.getFullYear() && 
            ...
            // Trigger the date click to load events
            setTimeout(() => {
                handleDateClickInternal(todayDate);
            }, 100);
        }
    }, [events]); // Run when events are loaded
                        ...
                        <div className="space-y-4">
                            {Object.keys(groupedEvents).length > 0 ? (
                                Object.entries(groupedEvents).map(([staffName, data]) => {
                                    ...
                                    const { events: staffEvents, hasLeave, hasOnDuty } = data;

                                    let cardBg = 'bg-gray-50';
                                    let cardBorder = 'border-gray-300';
                                    if (hasLeave && hasOnDuty) {
                                        cardBg = 'bg-purple-50';
                                                ...
                                                <div className="p-3 border-t border-gray-200 space-y-3">
                                                    {staffEvents.map((event, idx) => {
                                                        ...
                                                        if (event.status === 'Approved') {
                                                            statusBg = event.type === 'leave' ? 'bg-blue-200' : 'bg-green-200';
                                                            statusText = event.type === 'leave' ? 'text-blue-800' : 'text-green-800';
                                                            ...
                                                            <div key={idx} className="bg-white p-2 rounded-md shadow-sm">
                                                                <div className="flex items-center justify-between">
                                                                    <p className="font-semibold text-xs text-gray-800">
                                                                        {event.type === 'leave' ? `Leave: ${event.title}` : 'On-Duty'}
                                                                    </p>
